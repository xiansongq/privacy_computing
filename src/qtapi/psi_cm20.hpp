...
void static  localpsireceiver(NetIO &io,OTEOPRF::PP pp, std::vector <block> vecx, Message &message, int slen) ;

//创建本地执行 psi 的api 需要两个函数 一边模拟 receiver 一边模拟 sender
inline void static localpsisender(NetIO &io,OTEOPRF::PP pp, std::vector <block> vecy, Message &message) ;
