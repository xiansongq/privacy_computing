...
NetIO::NetIO(std::string party, std::string address, int port) {
    ...
    if (party == "server") {
        ...
        int reuse = 1;
        if (setsockopt(this->server_master_socket, SOL_SOCKET, SO_REUSEADDR, (const char *) &reuse, sizeof(reuse)) <
            0) {
            throw std::logic_error("error: setsockopt");
        ...
        if (bind(this->server_master_socket, (struct sockaddr *) &server_address, sizeof(struct sockaddr)) < 0) {
            throw std::logic_error("error: fail to bind server master socket");
        ...
        if (listen(this->server_master_socket, 1) < 0) {
            throw std::logic_error("error: server master socket fail to listen");
        ...
        if (connect_socket < 0) {
            throw std::logic_error("error: fail to accept client socket");
        ...
        if (connect(this->connect_socket, (struct sockaddr *) &server_address, sizeof(struct sockaddr_in)) < 0) {
            throw std::logic_error("error :connect first start server");
