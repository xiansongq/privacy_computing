...
namespace SEALPIR_CLIENT {
    ...
    void setup (const EncryptionParameters &enc_params,
                         const SEALPIR::PirParams &pir_params)
    ...
    SEALPIR::PirQuery  generate_query(uint64_t desiredIndex) {

        indices_ = SEALPIR::compute_indices(desiredIndex, pir_params_.nvec);

        SEALPIR::PirQuery result(pir_params_.d);
        int N = enc_params_.poly_modulus_degree();
    ...
    vector<uint8_t>  decode_reply(SEALPIR::PirReply &reply, uint64_t offset) {
    ...
    std::vector<uint64_t>  extract_coeffs(seal::Plaintext pt,
        ...
        uint64_t coeffs_per_element =
                SEALPIR::coefficients_per_element(logt, pir_params_.ele_size);
    ...
    std::vector<uint8_t>  extract_bytes(seal::Plaintext pt,
        ...
        vector<uint64_t> coeffs;
        encoder_->decode(pt, coeffs);
        SEALPIR::coeffs_to_bytes(logt, coeffs, elems.data(), bytes_per_ptxt,
    ...
    Plaintext  decode_reply(SEALPIR::PirReply &reply) {
        ...
        uint32_t exp_ratio = SEALPIR::compute_expansion_ratio(parms);
      ...
      cout << decryptor_->invariant_noise_budget(temp[j]) << endl;
                ...
                if ((j + 1) % (exp_ratio * ciphertext_size) == 0 && j > 0) {
                    ...
                    SEALPIR::compose_to_ciphertext(parms, tempplain, combined);
