...
// Number of FV plaintexts needed to represent the database
    ...
    void vector<::uint8_t> Client(NetIO &io, PirParams pirParams, std::vector<block> &vec_Y){
    cout << "Main: Generating galois keys for client" << endl;
    GaloisKeys galois_keys = client.generate_galois_keys();
    // 发送 galois_key 给server
    io.
    }
    void PIRClient(const EncryptionParameters &enc_params,
