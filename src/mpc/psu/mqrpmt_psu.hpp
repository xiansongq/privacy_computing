...
namespace mqRPMTPSU {
    ...
    PP Setup(std::string filter_type,
             ...
             size_t LOG_SENDER_LEN, size_t LOG_RECEIVER_LEN,size_t SLEN) {
        PP pp;
...
// load pp from file
    ...
    Message Send(NetIO &io, PP &pp, std::vector <block> &vec_X) {
        Message message;
...
//            exit(1);
    ...
    Message Receive(NetIO &io, PP &pp, std::vector <block> &vec_Y) {
        Message message;
...
//        }
        ...
        for(int i=0;i<vec_Y.size()-pp.slen;i++) ans.push_back(std::to_string(Block::BlockToInt64(vec_Y[i])));
        for (int i = 0; i < vec_X_diff.size(); i++) {
             ans.push_back(std::to_string(Block::BlockToInt64(vec_X_diff[i])));
        }
        // 存在返回结果是负数的情况 可能是因为他是 SSE多指令处理吧
        message.data = ans;
        message.code = 200;
        message.msg = "执行成功";
...
#pragma omp parallel for num_threads(thread_count)
    ...
    Receive(NetIO &io, PP &pp, std::vector <std::vector<uint8_t>> &vec_Y, size_t ITEM_LEN) {
        if (vec_Y.size() != pp.RECEIVER_LEN) {
            std::cerr << "|Y| does not match public parameter" << std::endl;
            exit(1);
        }
