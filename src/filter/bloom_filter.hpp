...
// insert vector<uint8_t>
#ifdef IS_MACOS
    template <>
#endif

    inline void Insert(const std::vector<std::vector<std::uint8_t>> &str) {
#pragma omp parallel for num_threads(thread_count)
...
#pragma omp parallel for num_threads(thread_count)
    ...
    inline std::vector<uint8_t> Contain(const std::vector<std::vector<std::uint8_t>> &vec_A) {
        size_t LEN = vec_A.size();
        std::vector<uint8_t> vec_indication_bit(LEN);
...
#pragma omp parallel for num_threads(thread_count)
        for (auto i = 0; i < vec_A.size(); i++) {
            ...
            if (PlainContain(buffer, vec_A[i].size()) == true) vec_indication_bit[i] = 1;
            else vec_indication_bit[i] = 0;
        }
        return vec_indication_bit;
    }
